...
namespace {
constexpr uint8_t READER_FONT_SCALE_VERSION = 2;
}
...
uint8_t migrateFontSizePresetV2(const uint8_t sizeValue) {
  const uint8_t shiftedSize = static_cast<uint8_t>(sizeValue + CrossPointSettings::FONT_SIZE_STEP);
  return normalizeFontSize(shiftedSize);
}
...
bool JsonSettingsIO::saveSettings(const CrossPointSettings& s, const char* path) {
  ...
  doc["readerFontScaleVersion"] = READER_FONT_SCALE_VERSION;
...
bool JsonSettingsIO::loadSettings(CrossPointSettings& s, const char* json, bool* needsResave) {
  ...
  const uint8_t readerFontScaleVersion = doc["readerFontScaleVersion"] | (uint8_t)1;
  const bool migrateReaderFontScale = !legacyFontEncoding && readerFontScaleVersion < READER_FONT_SCALE_VERSION;
  if (migrateReaderFontScale && needsResave) *needsResave = true;
    ...
    if (info.stringOffset) {
      ...
      if (info.valuePtr == &CrossPointSettings::fontSize) {
        if (legacyFontEncoding) {
          ...
          if (needsResave) *needsResave = true;
        } else if (migrateReaderFontScale) {
          v = migrateFontSizePresetV2(v);
          if (needsResave) *needsResave = true;
        } else {
      ...
      if (info.valuePtr == &CrossPointSettings::motionPageTurn && v == 6) {
        // Legacy hidden "Tilt L/R reversed" option is now the single Tilt L/R mode.
        v = CrossPointSettings::MOTION_TILT_LR;
        if (needsResave) *needsResave = true;
      }
