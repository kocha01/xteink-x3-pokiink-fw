...
void UITheme::setTheme(CrossPointSettings::UI_THEME type) {
  // Halo 2 is the only theme
  (void)type;
  LOG_DBG("UI", "Using Halo 2 theme");
...
UIIcon UITheme::getFileIcon(const std::string& filename) {
  ...
  if (FsHelpers::hasBmpExtension(filename) || FsHelpers::hasXtgExtension(filename) || FsHelpers::hasXthExtension(filename)) {
    return Image;
  }
  return File;
}
