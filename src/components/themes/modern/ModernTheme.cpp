...
#include <Arduino.h>  // millis() — used for per-cover render timing logs
#include <algorithm>
...
// row-by-row pixel reads ≈ 500-750 ms per nav, on top of the ~400 ms FAST_REFRESH
...
const std::vector<uint8_t>* loadOrCacheBmp(const std::string& path) {
  ...
  // Heap-aware fallback: bail out only if there isn't even enough room for
  // the file's bytes + a small allocator overhead margin.  Previously this
  // margin was 40 KB to match the X4 fork — that was way too aggressive on
  // X3 where a 260-px-tall center cover BMP is ~25-30 KB, so a heap with
  // 60 KB free (typical post-reader-exit with Wi-Fi still running) failed
  // the check and the center cover dropped to the black-placeholder path.
  // Users saw the carousel center "go blank".  8 KB matches the smaller-
  // buffer margin used by FontDecompressor's hotGlyphBuf pre-flight and is
  // empirically enough to cover std::vector::resize's bookkeeping +
  // alignment slop without spuriously rejecting valid loads.
  // (See docs/x3-backport-handoff.md Bug 8.)
  constexpr size_t BMP_CACHE_MIN_FREE_HEAP_BYTES = 8 * 1024;
...
void ModernTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle) const {
  ...
  if (NTP_CLOCK.isSynced()) {
    renderer.drawText(SMALL_FONT_ID, rect.x + ModernMetrics::values.contentSidePadding, rect.y + 5,
                      NTP_CLOCK.getTimeString());
  }
...
void ModernTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  if (!bufferRestored) {
    ...
    auto drawCoverAt = [&](int idx, int centerAnchorX, int topY, int targetH, int fallbackW, int maxW, int thumbHeight,
                           int outlineWidth) {
      // Per-cover timing breakdown: `cacheMs` covers the SD-or-RAM lookup
      // (~0 ms on cache hit, ~30-150 ms on miss depending on BMP size);
      // `parseMs` is the BMP header parse (~1 ms); `drawMs` is the actual
      // pixel-bashing into the framebuffer including 2bpp quantization.
      // Lets us see which thumbHeight (150/200/260) is the slow one when
      // the user reports laggy carousel nav.
      const unsigned long tCoverStart = millis();
      int displayW = fallbackW;
      const std::string& coverPath = recentBooks[idx].coverBmpPath;
      // Shared rendering closure — accepts either a memory-backed or file-
      // backed Bitmap so we can fall back from cache → direct SD when the
      // heap is too tight to admit another cache entry.  Without this fall-
      // through the center cover (largest, allocated last) drops to the
      // placeholder card whenever `loadOrCacheBmp` returns nullptr — users
      // see "center cover missing" intermittently after several navs even
      // though the BMP file is fine on disk.
      auto drawCard = [&](Bitmap& bmp) -> int {
        if (bmp.parseHeaders() != BmpReaderError::Ok) return -1;
        const int w = displayWidthForBitmap(bmp, targetH, fallbackW, maxW);
        const int cardX = centerAnchorX - w / 2 - coverBorder;
        const int cardY = topY - coverBorder;
        const int cardW = w + coverBorder * 2;
      ...
      if (!coverPath.empty()) {
        ...
        // Try the cache first; fall back to a direct SD read (uncached) when
        // the cache loader returned nullptr (heap pressure / file too large).
        const auto* cachedBytes = loadOrCacheBmp(bmpPath);
        const unsigned long tCoverCache = millis();
        if (cachedBytes) {
          Bitmap bmp(cachedBytes->data(), cachedBytes->size());
          const int w = drawCard(bmp);
          if (w >= 0) {
            const unsigned long tCoverDraw = millis();
            LOG_DBG("HOME", "cover idx=%d h=%d: cache=%lums draw=%lums total=%lums size=%u src=cached",
                    idx, thumbHeight, tCoverCache - tCoverStart, tCoverDraw - tCoverCache,
                    tCoverDraw - tCoverStart, static_cast<unsigned>(cachedBytes->size()));
            return w;
          }
        } else {
          // Fallback: read straight from SD without caching.  Slower
          // (~30-150 ms depending on BMP size) but always succeeds when the
          // file exists, so the visible carousel never has a "missing
          // middle" hole on heap-pressured frames.
          FsFile file;
          if (Storage.openFileForRead("HOME", bmpPath, file)) {
            ...
            if (w >= 0) {
              const unsigned long tCoverDraw = millis();
              LOG_DBG("HOME", "cover idx=%d h=%d: openFile+draw=%lums total=%lums src=direct-sd",
                      idx, thumbHeight, tCoverDraw - tCoverCache, tCoverDraw - tCoverStart);
              return w;
            }
          }
        }
  ...
  // --- LAYER 3: CENTER cover (on top, with white border) ---
  {
    ...
    // Must redraw whenever the framebuffer was cleared (bufferRestored=false).
    //
    // Bug history: setting `coverRendered = true` unconditionally at the end
    // of this block (regardless of whether `storeCoverBuffer()` succeeded)
    // produced a state where coverRendered=true AND coverBufferStored=false
    // after a heap-pressure malloc failure.  On the next render:
    //   clearScreen → framebuffer white
    //   restoreCoverBuffer fails (coverBufferStored=false) → bufferRestored=false
    //   Layer 1+2 redrawn (because !bufferRestored)
    //   Layer 3 SKIPPED (because coverRendered is still true)
    // → side covers visible, center cover gone, exactly matching the
    // user-reported "carousel center hole" that persisted after the heap-
    // margin tweak and the direct-SD fallback.  Forcing redraw whenever
    // bufferRestored==false is the only invariant that guarantees the
    // center cover is in the framebuffer when we hand it to the e-ink driver.
    if (!coverRendered || !bufferRestored) {
      ...
      if (!coverPath.empty()) {
        ...
        // Shared cover rendering: tries cache first, falls back to direct SD
        // load on cache miss/bail.  Without the direct-SD fallback the
        // center cover was vanishing intermittently after several carousel
        // navs — the cache's heap-aware guard returned nullptr under heap
        // pressure and the placeholder code drew a black rectangle that
        // looked, on e-ink, like the center cover had disappeared.
        auto drawCenter = [&](Bitmap& bitmap) -> bool {
          if (bitmap.parseHeaders() != BmpReaderError::Ok) return false;
          displayW =
              std::clamp(std::max(1, (bitmap.getWidth() * centerCoverH + bitmap.getHeight() / 2) / bitmap.getHeight()),
                         1, centerCoverH);
          ...
          renderer.drawRoundedRect(cardX, cardY, cardW, cardH, bookIsSelected ? 3 : coverCardOutline,
                                   coverCardRadius, true);
          coverWidth = displayW;
          return true;
        };

        // 1) Try the LRU cache (fast path, ~0 ms on hit).
        if (const auto* cachedBytes = loadOrCacheBmp(coverBmpPath)) {
          Bitmap bitmap(cachedBytes->data(), cachedBytes->size());
          drewCover = drawCenter(bitmap);
        }
        // 2) Fallback to direct SD read when cache miss/bail produced nullptr.
        //    Slower (~50-150 ms) but the cover always shows.
        if (!drewCover) {
          FsFile file;
      ...
      coverRendered = true;  // Cover is drawn; buffer store is an optimisation, not a prerequisite
    }
