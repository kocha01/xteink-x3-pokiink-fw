...
constexpr int headerClockGap = 12;
int getHeaderClockWidth(const GfxRenderer& renderer) {
  if (!NTP_CLOCK.isSynced()) {
    return 0;
  }

  return renderer.getTextWidth(SMALL_FONT_ID, NTP_CLOCK.getTimeString());
}

void drawHeaderClock(const GfxRenderer& renderer, const ThemeMetrics& metrics, const Rect rect) {
  if (!NTP_CLOCK.isSynced()) {
    return;
  }

  renderer.drawText(SMALL_FONT_ID, rect.x + metrics.contentSidePadding, rect.y + 5, NTP_CLOCK.getTimeString());
}
...
void BaseTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle) const {
  ...
  drawHeaderClock(renderer, BaseMetrics::values, rect);

  if (title) {
    const int clockWidth = getHeaderClockWidth(renderer);
    const int leftReserved = BaseMetrics::values.contentSidePadding + (clockWidth > 0 ? clockWidth + headerClockGap : 0);
    const int rightReserved = rect.width - batteryX + BaseMetrics::values.batteryWidth;
    const int availableWidth = rect.width - leftReserved - rightReserved - BaseMetrics::values.contentSidePadding;
    auto truncatedTitle =
        renderer.truncatedText(UI_12_FONT_ID, title, std::max(0, availableWidth), EpdFontFamily::BOLD);
    const int titleWidth = renderer.getTextWidth(UI_12_FONT_ID, truncatedTitle.c_str(), EpdFontFamily::BOLD);
    const int titleX = leftReserved + std::max(0, (availableWidth - titleWidth) / 2);
    renderer.drawText(UI_12_FONT_ID, rect.x + titleX, rect.y + 5, truncatedTitle.c_str(), true, EpdFontFamily::BOLD);
  }
...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
  ...
  const int batteryBlockWidth = SETTINGS.statusBarBattery
                                    ? metrics.batteryWidth +
                                          (showBatteryPercentage
                                               ...
                                               : 0)
                                    : 0;
  const int leftBaseX = orientedMarginLeft + metrics.contentSidePadding;
  ...
  if (!title.empty()) {
    ...
    const int batterySize = SETTINGS.statusBarBattery ? batteryBlockWidth : 0;
