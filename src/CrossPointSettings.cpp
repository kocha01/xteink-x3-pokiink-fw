...
bool CrossPointSettings::loadFromBinaryFile() {
  ...
  do {
    ...
    if (++settingsRead >= fileSettingsCount) break;
    {
      // SD card font family name (variable-length string, max 31 chars + NUL)
      std::string sdName;
      serialization::readString(inputFile, sdName);
      strncpy(sdFontFamilyName, sdName.c_str(), sizeof(sdFontFamilyName) - 1);
...
int CrossPointSettings::getReaderFontId() const {
  // SD card font takes precedence when a family is selected and registry resolved.
  // Falls through to built-in family if the SD font isn't currently loaded.
  ...
  switch (fontFamily) {
    ...
    default:
      switch (normalizeFontSize(fontSize)) {
        ...
        case FONT_20: return BAIJAMJUREE_20_FONT_ID;
        case FONT_22:
        default:
          return BAIJAMJUREE_22_FONT_ID;
      }
    case CLOUDLOOP:
      switch (normalizeFontSize(fontSize)) {
        ...
        case FONT_20: return CLOUDLOOP_20_FONT_ID;
        case FONT_22:
        default:
          return CLOUDLOOP_22_FONT_ID;
      }
    case BOOKERLY:
      switch (normalizeFontSize(fontSize)) {
        ...
        case FONT_20: return BOOKERLY_20_FONT_ID;
        case FONT_22:
        default:
...
int CrossPointSettings::getThaiFallbackFontId() const {
  ...
  switch (normalizeFontSize(fontSize)) {
    ...
    case FONT_20: return NOTOSERIF_20_FONT_ID;
    case FONT_22:
    default:
...
int CrossPointSettings::getReaderFontIdForLanguage(const std::string& language) const {
  // SD font is an explicit user choice and wins over the Bookerly→Thai-fallback
  // heuristic.  Without this, picking a custom Thai font on a device whose
  // built-in fontFamily is still BOOKERLY silently routes Thai pages back to
  // the built-in Noto Serif Thai fallback instead of the user's selection.
  // NOTE: must use `id != 0` not `id > 0` — SD font IDs are FNV-1a hashes cast
  // to int32_t and CAN BE NEGATIVE (e.g. ChakraPetch hashes to -2119622908).
  if (sdFontFamilyName[0] != '\0' && sdFontIdResolver) {
    int id = sdFontIdResolver(sdFontResolverCtx, sdFontFamilyName, fontSize);
    if (id != 0) return id;
  }
...
int CrossPointSettings::getReaderFontIdForThaiContent(const std::string& language, const std::string& title) const {
  // SD font wins over every built-in heuristic — see comment in
  // getReaderFontIdForLanguage().  Same `id != 0` negative-hash guard applies.
  if (sdFontFamilyName[0] != '\0' && sdFontIdResolver) {
    int id = sdFontIdResolver(sdFontResolverCtx, sdFontFamilyName, fontSize);
    if (id != 0) return id;
  }
