...
namespace {
// Available reader font sizes for the *currently active* font family:
//   - SD font selected → return the actual .cpfont sizes on disk for that family
//     (e.g. 18/20/22/24/26 for a custom font compiled at those sizes).
//   - Built-in font selected (or no SD font discovered) → 14/16/18/20/22.
// Result is sorted ascending and de-duplicated.  Always returns at least the
// built-in list — never empty.
std::vector<uint8_t> availableReaderFontSizes() {
  ...
  if (sizes.empty()) {
    for (uint8_t s = CrossPointSettings::FONT_SIZE_MIN; s <= CrossPointSettings::FONT_SIZE_MAX;
         s += CrossPointSettings::FONT_SIZE_STEP) {
      sizes.push_back(s);
    }
  }
  return sizes;
...
uint8_t nextReaderFontSize(uint8_t currentValue) {
  const auto sizes = availableReaderFontSizes();
  // Find the smallest size strictly greater than currentValue; wrap to first.
  for (uint8_t s : sizes) {
...
uint8_t prevReaderFontSize(uint8_t currentValue) {
  const auto sizes = availableReaderFontSizes();
  // Find the largest size strictly less than currentValue; wrap to last.
  for (auto it = sizes.rbegin(); it != sizes.rend(); ++it) {
    if (*it < currentValue) return *it;
  }
  return sizes.back();
}
...
std::vector<EpubReaderMenuActivity::MenuItem> EpubReaderMenuActivity::buildFullMenuItems(bool hasFootnotes) {
  ...
  items.push_back({MenuAction::BACKGROUND_TEXTURE, StrId::STR_SCREENSHOT_BUTTON});  // TODO: add i18n key
...
void EpubReaderMenuActivity::loop() {
  ...
  if (currentMode == Mode::QUICK) {
    // ══ QUICK SETTINGS MODE ══
    // Power = cycle font family.  When the user is on a custom SD font, POWER
    // is a no-op — Quick Settings becomes "size only" so an accidental press
    // doesn't yank the user out of their chosen custom font.  To switch back
    // to a built-in family, the user goes through Settings → Reader → Custom
    // Font (SD) → "(Built-in)".  When on a built-in family, POWER cycles
    // through the 3 built-in families like the original behaviour.
    if (mappedInput.wasReleased(MappedInputManager::Button::Power)) {
      if (pendingSdFontFamilyName.empty()) {
    ...
    if (mappedInput.wasReleased(MappedInputManager::Button::Down)) {
      ...
      arrowPressTime = millis();
      arrowResetPending = true;
      // Reverse cycle through whatever sizes the active font family actually
      // has on disk (built-in 14/16/18/20/22 or SD font's compiled sizes).
      pendingFontSize = prevReaderFontSize(pendingFontSize);
      requestUpdate();
      return;
    }
    ...
    if (mappedInput.wasReleased(MappedInputManager::Button::Back)) {
      ...
      result.data = MenuResult{-1, pendingOrientation, selectedPageTurnOption, pendingFontFamily, pendingFontSize,
                               pendingLineSpacing, pendingScreenMargin, pendingSdFontFamilyName};
...
void EpubReaderMenuActivity::renderQuickSettings(int x, int /*y*/, int w, int /*h*/) const {
  ...
  const char* dictLabel = "Dictionary";
  renderer.drawText(SMALL_FONT_ID,
                    orBoxX + (halfBoxW - renderer.getTextWidth(SMALL_FONT_ID, dictLabel)) / 2,
                    dmMid - 10, dictLabel, true);
...
const char* EpubReaderMenuActivity::getItemValue(MenuAction action) const {
  ...
  switch (action) {
    case MenuAction::FONT_FAMILY: {
      // SD font selected → show its discovered family name (registry uses the
      // directory name, which is what the user dropped onto the SD card).
      if (!pendingSdFontFamilyName.empty()) {
        return pendingSdFontFamilyName.c_str();
      }
      StrId label;
