...
int getReaderTopClockReserve() {
  if (!NTP_CLOCK.isSynced()) {
    return 0;
  }

  return UITheme::getInstance().getMetrics().homeTopPadding;
}

void drawReaderTopClock(const GfxRenderer& renderer) {
  if (!NTP_CLOCK.isSynced()) {
    return;
  }

  const auto& metrics = UITheme::getInstance().getMetrics();
  renderer.drawText(SMALL_FONT_ID, metrics.contentSidePadding, metrics.topPadding + 5, NTP_CLOCK.getTimeString());
}
...
void EpubReaderActivity::onExit() {
  ...
  // Free SD font per-page glyph caches before Home re-renders allocate its
  // 96 KB cover BMP cache.  Without this, Reader → Home on a device with a
  // loaded SD font can OOM and reboot when the mini-cache (~17-30 KB) and
  // the cover cache both want the same heap.  Font header + intervals stay
  // resident so the font is still selected; the mini-cache rebuilds on the
  // first page-turn next time the user opens a book.
  ...
  switch (action) {
    ...
    case EpubReaderMenuActivity::MenuAction::THAI_DICTIONARY: {
      startActivityForResult(std::make_unique<ThaiDictionaryActivity>(renderer, mappedInput),
                             [this](const ActivityResult&) {
                               // Dictionary may have changed — invalidate section so cache is re-checked
                               RenderLock lock(*this);
                               section.reset();
                             });
      break;
...
void EpubReaderActivity::applyMenuSettings(const MenuResult& menu, const uint8_t boldBefore) {
  ...
  const bool sdFontFamilyChanged = (menu.sdFontFamilyName != std::string(SETTINGS.sdFontFamilyName));

  if (!orientationChanged && !fontFamilyChanged && !fontSizeChanged && !lineSpacingChanged && !screenMarginChanged &&
      !boldChanged && !sdFontFamilyChanged) {
    return;
  }

  {
    ...
    if (sdFontFamilyChanged) {
      // Truncate to settings buffer size; reader-menu is the user's intent so
      // mismatched name lengths shouldn't silently revert.
      std::snprintf(SETTINGS.sdFontFamilyName, sizeof(SETTINGS.sdFontFamilyName), "%s",
                    menu.sdFontFamilyName.c_str());
      ensureSdFontLoaded();  // load/unload the right .cpfont family
    }
    SETTINGS.saveToFile();
...
void EpubReaderActivity::render(RenderLock&& lock) {
  ...
  orientedMarginTop += getReaderTopClockReserve();
...
void EpubReaderActivity::renderContents(std::unique_ptr<Page> page, const int orientedMarginTop,
  ...
  const bool useTextAntiAliasing = ReaderUtils::shouldUseTextAntiAliasing();
  bool imagePageWithAA = page->hasImages() && useTextAntiAliasing;
