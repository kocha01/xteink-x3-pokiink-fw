...
struct MenuResult {
  ...
  // SD card font family name. Empty = built-in fontFamily applies; non-empty
  // overrides via SdCardFontSystem.  Set by EpubReaderMenuActivity when the
  // user cycles POWER through a discovered SD family.
...
struct XtCloudResult {
  bool shouldOpenPairing = false;
};

using ResultVariant = std::variant<std::monostate, WifiResult, KeyboardResult, MenuResult, ChapterResult, PercentResult,
                                   PageResult, SyncResult, NetworkModeResult, FootnoteResult, XtCloudResult>;
