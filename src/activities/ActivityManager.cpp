...
// Defined in main.cpp — loads reader fonts on first use
extern void ensureReaderFonts();
...
void ActivityManager::goToReader(std::string path) {
  // Lazy-load reader fonts on first book open (deferred from boot for faster startup)
  ensureReaderFonts();
...
void ActivityManager::pushToSleep() {
  // Push SleepActivity on top of the current activity so the current one (e.g. HomeActivity)
  // stays alive in SRAM across light sleep. Wake path pops back to restore without rebuilding.
  pushActivity(std::make_unique<SleepActivity>(renderer, mappedInput));
  loop();  // Render the sleep screen now — caller enters light sleep right after
}
