...
void CalibreConnectActivity::render(RenderLock&&) {
  ...
  if (state == CalibreConnectState::WIFI_SELECTION || state == CalibreConnectState::SERVER_STARTING) {
    // Initial state on entry is WIFI_SELECTION — the activity calls
    // requestUpdate() BEFORE deciding whether to launch the wifi picker, so
    // the screen would briefly render with no body text at all (just the
    // header) before SERVER_STARTING took over.  On slow e-ink that left
    // a blank ~400 ms frame that users read as "page is broken — no text".
    // Treat WIFI_SELECTION the same as SERVER_STARTING in the render — both
    // are "we're getting ready, hang tight" states.
    ...
    // Show SSID + IP as two stacked SMALL_FONT_ID rows instead of squeezing
    // both into drawSubHeader's right-label slot.  drawSubHeader caps the
    // right label at maxListValueWidth (~200 px); on X3 "IP Address: " +
    // a long IP like "192.168.100.100" easily exceeds that and the tail
    // gets ellipsized, hiding the last octet — the user can't see their
    // own IP.  Stacking them at SMALL_FONT_ID on a full-width row gives
    // each value the entire content width.
    const int subTop = metrics.topPadding + metrics.headerHeight + metrics.verticalSpacing;
    const auto smallLineHeight = renderer.getLineHeight(SMALL_FONT_ID);
    renderer.drawText(SMALL_FONT_ID, metrics.contentSidePadding, subTop,
                      connectedSSID.c_str(), true, EpdFontFamily::BOLD);
    const std::string ipLine = std::string(tr(STR_IP_ADDRESS_PREFIX)) + connectedIP;
    renderer.drawText(SMALL_FONT_ID, metrics.contentSidePadding, subTop + smallLineHeight,
                      ipLine.c_str());
    // Underline separator like drawSubHeader would have drawn.
    const int separatorY = subTop + smallLineHeight * 2 + metrics.verticalSpacing;
    renderer.drawLine(0, separatorY, pageWidth - 1, separatorY, true);

    int y = separatorY + metrics.verticalSpacing * 4;
