...
#include "activities/network/XtCloudActivity.h"
#include "activities/network/XteinkAppConnectionActivity.h"
...
constexpr bool kXteinkFeaturesEnabled = false;
}  // namespace
...
void CrossPointWebServerActivity::reopenNetworkModeSelection() {
  startActivityForResult(std::make_unique<NetworkModeSelectionActivity>(renderer, mappedInput),
                         [this](const ActivityResult& result) {
                           if (result.isCancelled) {
                             onGoHome();
...
void CrossPointWebServerActivity::onNetworkModeSelected(const NetworkMode mode) {
  ...
  } else if (mode == NetworkMode::XTEINK_APP) {
    modeName = "XTEINK App Connection";
  } else if (mode == NetworkMode::XT_CLOUD) {
    modeName = "XT Cloud";
  }
  LOG_DBG("WEBACT", "Network mode selected: %s", modeName);

  if (!kXteinkFeaturesEnabled &&
      (mode == NetworkMode::XTEINK_APP || mode == NetworkMode::XT_CLOUD)) {
    LOG_DBG("WEBACT", "XTEINK App / XT Cloud are temporarily disabled; returning to mode selection");
    state = WebServerActivityState::MODE_SELECTION;
    reopenNetworkModeSelection();
    return;
  }
  ...
  if (mode == NetworkMode::XTEINK_APP) {
    state = WebServerActivityState::MODE_SELECTION;
    startActivityForResult(std::make_unique<XteinkAppConnectionActivity>(renderer, mappedInput),
                           [this](const ActivityResult&) { reopenNetworkModeSelection(); });
    return;
  }

  if (mode == NetworkMode::XT_CLOUD) {
    state = WebServerActivityState::MODE_SELECTION;
    startActivityForResult(std::make_unique<XtCloudActivity>(renderer, mappedInput), [this](const ActivityResult& result) {
      const bool shouldOpenPairing =
          !result.isCancelled && std::holds_alternative<XtCloudResult>(result.data) &&
          std::get<XtCloudResult>(result.data).shouldOpenPairing;
...
void CrossPointWebServerActivity::renderServerRunning() const {
  ...
  if (isApMode) {
    ...
    std::string mdnsUrl = std::string(tr(STR_OR_HTTP_PREFIX)) + AP_HOSTNAME + ".local/";
    renderer.drawCenteredText(SMALL_FONT_ID, startY, mdnsUrl.c_str(), true);
  }
