...
int ThaiDictionaryActivity::totalItems() const {
  // First item is always "Add Word", followed by existing words
  return 1 + static_cast<int>(words.size());
}
...
void ThaiDictionaryActivity::deleteSelectedWord() {
  const int wordIndex = selectedIndex - 1;  // offset by "Add Word" entry
...
void ThaiDictionaryActivity::render(RenderLock&&) {
  ...
  GUI.drawList(
      ...
      [this](const int index) -> std::string {
        if (index == 0) return std::string("+ ") + tr(STR_ADD_WORD);
        const int wordIdx = index - 1;
  ...
  // Button hints
  const char* confirmLabel = selectedIndex == 0 ? tr(STR_ADD_WORD) : tr(STR_DELETE);
