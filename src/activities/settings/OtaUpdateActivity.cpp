...
void OtaUpdateActivity::onWifiSelectionComplete(const bool success) {
  ...
  if (res == OtaUpdater::NO_UPDATE) {
    // 404 from GitHub (repo missing / no published release / drafts only).
    // Per the OtaUpdater comment above, this is semantically "you're on the
    // newest available binary" — show the friendly STR_NO_UPDATE message
    // instead of the alarming STR_UPDATE_FAILED.
    LOG_DBG("OTA", "No release available yet — showing 'up to date'");
    {
      RenderLock lock(*this);
      state = NO_UPDATE;
    }
