...
class CrossPointWebServer {
 ...
 private:
  ...
  void handleXteinkStatus() const;
  void handleXteinkList() const;
  void handleXteinkEditUpload(UploadState& state) const;
  void handleXteinkEditPost(UploadState& state) const;
  void handleXteinkEditPut() const;
  void handleXteinkEditDelete() const;
  bool handleXteinkFileRequest() const;
  ...
  // v4 naming convention `<Family>_<size>.cpfont` (e.g. PKNakhonSawan_18.cpfont).
  ...
  // if present), then triggers a registry rescan.  Closes the upload-only
  // loop so the web UI can fully manage SD fonts without an SD eject.
