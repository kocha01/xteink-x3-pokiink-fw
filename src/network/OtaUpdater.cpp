...
namespace {
// PokiInk-X3 release feed.  Was previously pointing at the upstream
// crosspoint-halo2-custom repo (X4 binaries) which bricked X3 devices when
// users tapped "Check for update" — the downloaded firmware.bin was built for
// X4's pinout/SD/keyboard wiring and panicked on boot.  Combined with the
// POKIINK_X3_FW_MAGIC scan in installUpdate() below, this gives us URL-level
// segregation plus a content-level board-tag check.
constexpr char latestReleaseUrl[] = "https://api.github.com/repos/kocha01/xteink-x3-pokiink-fw/releases/latest";
...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  ...
  /*
   * Treat "repo exists but has no release yet" as NO_UPDATE rather than a
   * hard failure.  GitHub returns 404 with body {"message":"Not Found",...}
   * for `releases/latest` whenever:
   *   - the repo doesn't exist
   *   - the repo exists but has zero published releases
   *   - all releases are drafts/pre-releases
   * In every one of those cases the user is necessarily already on the
   * newest binary that exists, so the right UX is "you're up to date" — not
   * the scary "Update failed" that used to show because the missing
   * tag_name field tripped the JSON_PARSE_ERROR branch below.
   */
  const int http_status = esp_http_client_get_status_code(client_handle);
  ...
  if (http_status == 404) {
    LOG_INF("OTA", "GitHub returned 404 — no published release yet, treating as up-to-date");
    return NO_UPDATE;
  }
  if (http_status < 200 || http_status >= 300) {
    LOG_ERR("OTA", "GitHub returned HTTP %d", http_status);
    return HTTP_ERROR;
  }
...
OtaUpdater::OtaUpdaterError OtaUpdater::installUpdate(void (*onProgress)(void*), void* progressCtx) {
  ...
  /*
   * PokiInk-X3 board-tag check.
   *
   * The downloaded binary passed the generic ESP32 image-header + app_desc
   * structural checks above, but those tell us nothing about which board
   * the firmware was built for.  An X4 binary has identical magic words and
   * would happily commit at this point, then panic on boot because of
   * mismatched GPIO pin definitions (SD CS, EPD pins, keyboard wiring).
   *
   * Defense: mmap the first 256 KB of the just-written OTA partition and
   * scan for the POKIINK_X3_FW_MAGIC sentinel.  The sentinel is embedded
   * in this firmware's .flash.rodata via PokiInkFwMagic.cpp, so any
   * PokiInk-X3 build (current or future) carries it.  An X4 build (or any
   * other unrelated ESP32 binary) won't.  We use mmap so the scan is a
   * direct memory read through the flash cache — fast (~few hundred ms for
   * 256 KB on ESP32-C3).
   *
   * If the marker is absent we abort the OTA without flipping the otadata
   * pointer — the device keeps booting the previous (known-good) slot.
   */
  {
    constexpr size_t SCAN_SIZE = 256 * 1024;
    const void* mapped_ptr = nullptr;
    esp_partition_mmap_handle_t mmap_handle = 0;
    const esp_err_t mmap_err = esp_partition_mmap(ota_part, 0, SCAN_SIZE,
                                                  ESP_PARTITION_MMAP_DATA,
                                                  &mapped_ptr, &mmap_handle);
    bool magicFound = false;
    if (mmap_err == ESP_OK && mapped_ptr != nullptr) {
      const uint8_t* haystack = static_cast<const uint8_t*>(mapped_ptr);
      const uint8_t needle0 = static_cast<uint8_t>(POKIINK_X3_FW_MAGIC[0]);
      // Manual byte-search rather than memmem (not always available in the
      // ESP-IDF C++ standard library).  The first-byte check makes the
      // inner memcmp rare enough that this scans 256 KB in well under a
      // second even on the C3's 160 MHz CPU.
      for (size_t i = 0; i + POKIINK_X3_FW_MAGIC_LEN <= SCAN_SIZE; ++i) {
        if (haystack[i] != needle0) continue;
        if (memcmp(haystack + i, POKIINK_X3_FW_MAGIC,
      ...
      LOG_ERR("OTA", "esp_partition_mmap failed for board-tag scan: %s",
              esp_err_to_name(mmap_err));
    }

    if (!magicFound) {
      LOG_ERR("OTA",
              "PokiInk-X3 board tag NOT FOUND in downloaded firmware. "
              "Refusing to commit OTA — the downloaded binary appears to be "
              "for a different board (e.g. X4) and would brick this device.");
      esp_https_ota_abort(ota_handle);
      return INTERNAL_UPDATE_ERROR;
    }
    LOG_INF("OTA", "PokiInk-X3 board tag verified.");
  }
