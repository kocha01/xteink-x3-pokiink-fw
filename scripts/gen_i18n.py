...
DEFAULT_INCLUDED_LANGUAGE_CODES: Tuple[str, ...] = ("EN", "TH")
...
def load_translations(
    ...
    included_language_codes: Tuple[str, ...] = DEFAULT_INCLUDED_LANGUAGE_CODES,
) -> Tuple[List[str], List[str], List[str], Dict[str, List[str]]]:
    """
    Read selected YAML files in *translations_dir* and return:
    ...
    English is always first. By default we only compile English + Thai into
    the firmware and leave the other translation YAML files as source assets.
    """
    ...
    included_codes = {code.upper() for code in included_language_codes}
    parsed = {
        name: data
        for name, data in parsed.items()
        if data.get("_language_code", "").upper() in included_codes
    }

    if not parsed:
        raise ValueError(
            "No translations matched the included language codes: "
            f"{', '.join(sorted(included_codes))}"
        )
    ...
    if english_file is None:
        raise ValueError("No included YAML file with _language_code: EN found")
    ...
    print(
        "Loaded "
        f"{len(language_codes)} languages ({', '.join(language_codes)}), "
...
def main(translations_dir=None, output_dir=None) -> None:
    ...
    print(
        ...
        f"{', '.join(DEFAULT_INCLUDED_LANGUAGE_CODES)}"
    )
    print()
