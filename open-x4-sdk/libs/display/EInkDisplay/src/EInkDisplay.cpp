...
//
// Layout per 42-byte LUT (SSD/UC-style):
//   Byte 0       = Phase-1 VS byte (2 bits per sub-phase A/B/C/D)
//   Bytes 1..5   = Phase-1 timing: TP0, TP1, TP2, TP3, RP
//   Byte 6       = Phase-2 VS byte
//   Bytes 7..11  = Phase-2 timing: TP0, TP1, TP2, TP3, RP
//   Bytes 12..   = Phases 3..7 (unused here, all zero)
// The TP fields must match across all LUTs (vcom/ww/bw/wb/bb) so the
// controller scans every transition in lockstep.
...
void EInkDisplay::copyGrayscaleLsbBuffers(const uint8_t* lsbBuffer) {
  ...
  if (_x3Mode) {
    // X3 gray LUTs are labeled by transition: dark gray uses WW and light gray
    // uses BW. Keeping LSB in previous/0x10 and MSB in current/0x13 maps the
    // renderer's 2-bit font planes onto those LUT entries.
    uint8_t row[128];
...
void EInkDisplay::saveFrameBufferAsPBM(const char* filename) {
  ...
  // Original buffer: WxH (landscape) -> HxW (portrait)
  const int DISPLAY_WIDTH_LOCAL = displayWidth;    // runtime panel width
  const int DISPLAY_HEIGHT_LOCAL = displayHeight;  // runtime panel height
