...
void GfxRenderer::drawBitmap(const Bitmap& bitmap, const int x, const int y, const int maxWidth, const int maxHeight,
  ...
  const float croppedWidth = (1.0f - cropX) * bitmap.getWidth();
  const float croppedHeight = (1.0f - cropY) * bitmap.getHeight();

  if (maxWidth > 0 && croppedWidth > 0.0f) {
    const float widthScale = static_cast<float>(maxWidth) / croppedWidth;
    if (allowUpscale || widthScale < 1.0f) {
      scale = widthScale;
      isScaled = widthScale != 1.0f;
    }
  }
  if (maxHeight > 0 && croppedHeight > 0.0f) {
    const float heightScale = static_cast<float>(maxHeight) / croppedHeight;
    if (allowUpscale || heightScale < 1.0f) {
      scale = isScaled ? std::min(scale, heightScale) : heightScale;
      isScaled = heightScale != 1.0f || isScaled;
    }
  }
  ...
  for (int bmpY = 0; bmpY < (bitmap.getHeight() - cropPixY); bmpY++) {
    ...
    int screenYEnd = screenY;
    if (isUpscaled) {
      screenYEnd = y + static_cast<int>(std::floor((sourceY + 1) * scale)) - 1;
      screenYEnd = std::max(screenYEnd, screenY);
    }
    ...
    if (screenYEnd < 0 || screenY >= getScreenHeight()) {
      continue;
    }
    ...
    const int drawYStart = std::max(0, screenY);
    const int drawYEnd = std::min(getScreenHeight() - 1, screenYEnd);
    ...
    for (int bmpX = cropPixX; bmpX < bitmap.getWidth() - cropPixX; bmpX++) {
      ...
      if (isUpscaled) {
        screenXEnd = x + static_cast<int>(std::floor((sourceX + 1) * scale)) - 1;
        screenXEnd = std::max(screenXEnd, screenX);
      }
      ...
      const int drawXStart = std::max(0, screenX);
      const int drawXEnd = std::min(getScreenWidth() - 1, screenXEnd);

      const uint8_t val = outputRow[bmpX / 4] >> (6 - ((bmpX * 2) % 8)) & 0x3;

      for (int py = drawYStart; py <= drawYEnd; py++) {
        for (int px = drawXStart; px <= drawXEnd; px++) {
...
void GfxRenderer::drawBitmap1Bit(const Bitmap& bitmap, const int x, const int y, const int maxWidth,
  ...
  if (maxWidth > 0 && bitmap.getWidth() > 0) {
    const float widthScale = static_cast<float>(maxWidth) / static_cast<float>(bitmap.getWidth());
    if (allowUpscale || widthScale < 1.0f) {
      scale = widthScale;
      isScaled = widthScale != 1.0f;
    }
  ...
  for (int bmpY = 0; bmpY < bitmap.getHeight(); bmpY++) {
    ...
    int screenYEnd = screenY;
    if (isUpscaled) {
      screenYEnd = y + static_cast<int>(std::floor((bmpYOffset + 1) * scale)) - 1;
      screenYEnd = std::max(screenYEnd, screenY);
    }
    if (screenYEnd < 0 || screenY >= getScreenHeight()) {
      continue;
    ...
    for (int bmpX = 0; bmpX < bitmap.getWidth(); bmpX++) {
      ...
      int screenXEnd = screenX;
      if (isUpscaled) {
        screenXEnd = x + static_cast<int>(std::floor((bmpX + 1) * scale)) - 1;
        screenXEnd = std::max(screenXEnd, screenX);
      }
      if (screenX >= getScreenWidth()) {
        break;
...
void GfxRenderer::displayBuffer(const HalDisplay::RefreshMode refreshMode) const {
  ...
  if (hasNextRefreshOverride_) {
    // Take whichever mode is STRONGER (lower enum value in FULL/HALF/FAST).
    // A caller that explicitly passes FULL_REFRESH — e.g. BootActivity's
    // wake-path ghost-clear pass — must never be silently downgraded to
    // the transition override's HALF_REFRESH. Conversely, activities that
    // use the default FAST_REFRESH get upgraded to HALF so the previous
    // activity's content is fully cleared.
    if (static_cast<int>(nextRefreshOverride_) < static_cast<int>(effectiveMode)) {
      effectiveMode = nextRefreshOverride_;
    }
    hasNextRefreshOverride_ = false;
  }
...
int GfxRenderer::getScreenWidth() const {
  switch (orientation) {
    case Portrait:
    case PortraitInverted:
      return HalDisplay::LOGICAL_PORTRAIT_WIDTH;
    case LandscapeClockwise:
    case LandscapeCounterClockwise:
      return HalDisplay::LOGICAL_PORTRAIT_HEIGHT;
  }
  return HalDisplay::LOGICAL_PORTRAIT_WIDTH;
}
...
bool GfxRenderer::storeBwBuffer() {
  ...
  for (size_t i = 0; i < BW_BUFFER_NUM_CHUNKS; i++) {
    ...
    const size_t chunkSize = getBwBufferChunkSize(i);
  ...
  LOG_DBG("GFX", "Stored BW buffer in %zu chunks", BW_BUFFER_NUM_CHUNKS);
  return true;
}
