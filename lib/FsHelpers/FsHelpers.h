...
// Check for .xtg extension (case-insensitive)
bool hasXtgExtension(std::string_view fileName);
inline bool hasXtgExtension(const String& fileName) {
  return hasXtgExtension(std::string_view{fileName.c_str(), fileName.length()});
}

// Check for .xth extension (case-insensitive)
bool hasXthExtension(std::string_view fileName);
inline bool hasXthExtension(const String& fileName) {
  return hasXthExtension(std::string_view{fileName.c_str(), fileName.length()});
}
