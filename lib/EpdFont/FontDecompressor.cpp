...
const uint8_t* FontDecompressor::getBitmap(const EpdFontData* fontData, const EpdGlyph* glyph, uint32_t glyphIndex) {
  ...
  if (!(!hotGroup.empty() && hotGroupFont == fontData && hotGroupIndex == groupIndex)) {
    ...
    // Pre-flight heap before resize().  Under -fno-exceptions, std::vector
    // resize → operator new on OOM calls abort() rather than throwing —
    // panic backtrace ends in __terminate / panic_abort and the device
    // reboots mid-render.  The existing `if (hotGroup.empty())` post-check
    // was dead code because the abort came first.  Free the previous group
    // before measuring so we count only what's actually available.
    // (See docs/x3-backport-handoff.md Critical Bug 6.)
    hotGroup.clear();
    hotGroup.shrink_to_fit();

    constexpr uint32_t FDC_HEAP_SAFETY_MARGIN = 8 * 1024;
    const uint32_t freeHeap = ESP.getFreeHeap();
    if (freeHeap < group.uncompressedSize + FDC_HEAP_SAFETY_MARGIN) {
      LOG_ERR("FDC", "OOM avoided: hot group %u needs %u + %u margin, free=%u",
              groupIndex, group.uncompressedSize, FDC_HEAP_SAFETY_MARGIN, freeHeap);
      hotGroupFont = nullptr;
      hotGroupIndex = UINT16_MAX;
      stats.getBitmapTimeUs += micros() - tStart;
      return nullptr;
  ...
  // Same OOM pattern as the hot-group resize above — smaller buffer so a
  // 4 KB margin is enough.
  if (glyph->dataLength > hotGlyphBuf.size()) {
    constexpr uint32_t FDC_GLYPH_HEAP_SAFETY_MARGIN = 4 * 1024;
    const uint32_t freeHeap = ESP.getFreeHeap();
    if (freeHeap < glyph->dataLength + FDC_GLYPH_HEAP_SAFETY_MARGIN) {
      LOG_ERR("FDC", "OOM avoided: glyph buf needs %u + %u margin, free=%u",
              glyph->dataLength, FDC_GLYPH_HEAP_SAFETY_MARGIN, freeHeap);
      stats.getBitmapTimeUs += micros() - tStart;
      return nullptr;
    }
