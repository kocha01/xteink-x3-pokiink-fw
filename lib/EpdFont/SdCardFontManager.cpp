...
void SdCardFontManager::clearAllMiniCaches() {
  // Frees the per-page glyph + bitmap + mini kern matrix on every loaded SD
  // font WITHOUT unloading the font itself (header + intervals stay resident
  // for fast re-activation).  Recovers ~15-30 KB per loaded font.
  for (auto& lf : loaded_) {
