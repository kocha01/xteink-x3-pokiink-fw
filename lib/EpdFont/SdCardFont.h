...
class SdCardFont {
 ...
 private:
  ...
  // Idempotency: skip a redundant prewarm() if the same UTF-8 text was just
  // prewarmed with the same (or stronger) mode.  Without this, page-turn
  // pipelines that call prewarm both at layout-time (metadata-only) and at
  // render-scan-pass (full bitmaps) plus on a re-render pass for the same
  // page caused 3+ full SD I/O cycles per page (~10s on real hardware).
  // Cleared by clearCache() so font switches/page-data invalidations work.
  uint32_t lastPrewarmHash_ = 0;
  uint8_t lastPrewarmStyleMask_ = 0;
  bool lastPrewarmHadBitmaps_ = false;
