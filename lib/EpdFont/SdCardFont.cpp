...
int SdCardFont::prewarm(const char* utf8Text, uint8_t styleMask, bool metadataOnly) {
  ...
  // Idempotency short-circuit: if the same text has just been prewarmed for
  // the same (or stronger) mode and an inclusive style mask, the mini cache is
  // already valid — every glyph that the caller will look up is already in
  // memory.  Skip the SD reads entirely.  This collapses the typical 3-pass
  // pipeline (layout-time metadata, render scan-pass full, re-render full)
  // into a single SD read pass per page.
  unsigned long startMs = millis();
  uint32_t inputHash = utf8Text ? fnv1a(reinterpret_cast<const uint8_t*>(utf8Text), strlen(utf8Text)) : 0;
  if (inputHash != 0 && inputHash == lastPrewarmHash_ &&
      (styleMask & lastPrewarmStyleMask_) == styleMask &&
      (metadataOnly || lastPrewarmHadBitmaps_)) {
  ...
  // Record the idempotency state so subsequent prewarm() calls with the same
  // text + same (or weaker) mode can short-circuit before doing any SD I/O.
  if (inputHash != 0 && totalMissed == 0) {
    lastPrewarmHash_ = inputHash;
    if (lastPrewarmHadBitmaps_ && !metadataOnly) {
      lastPrewarmStyleMask_ |= styleMask;  // accumulate styles already cached
    } else {
      lastPrewarmStyleMask_ = styleMask;
    }
    lastPrewarmHadBitmaps_ = !metadataOnly;
  }
  stats_.prewarmTotalMs = millis() - startMs;
  return totalMissed;
}
...
void SdCardFont::clearCache() {
  ...
  // Invalidate the idempotency tracker — the mini cache is now empty so the
  // next prewarm() must actually perform SD reads.
  lastPrewarmHash_ = 0;
  lastPrewarmStyleMask_ = 0;
  lastPrewarmHadBitmaps_ = false;
}
